...
%detect the crop amount and load the features.
    ...
    GetCropAmount
    for nfeat = 1:tFeat;
        input{2*nfeat-1} = GetFeatMap(input{2*nfeat},ca);%store the featuremap
    end
...
function [fmap] = GetFeatMap(be,ca);
	fmap = Images2FeatMap( be.fmap.Path , be.fmap.ImIndex, ca);
    RESIZE;
    function RESIZE
	for ni = 1:size(fmap.data,2)
	dummy = reshape(fmap.data(:,ni),fmap.CurrentSize);
	dummy = imresize(dummy,fak);
	foo(:,ni) = dummy(:);
	end
	fmap.data = foo;
    end
