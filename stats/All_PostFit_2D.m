function All_PostFit_2D(FileList);
%All_PostFit_2D(FileList);
%
%
...
for nf = 1:tf;%RUN OVER ALL FILES/
    ...
    function [bc]=GetBinCenters(res);
        ...
        bc_    = be(1:end-1) + mean(diff(be))/2;
        [bc{1} bc{2}] = ndgrid(bc_);
        bc{1} = bc{1}(:);
        bc{2} = bc{2}(:);
    end
%
%
    function Column2Fit
        ...
        for nvec = 1:size(res.(nf{1}).(field{nf2}),2);
            [b,bi,dummy,dummy2,stats] = regress( res.(nf{1}).(field{nf2})(:,nvec) , indep );
            S.r(nvec)         = stats(1);
            S.F(nvec)         = stats(2);
            S.p(nvec)         = stats(3);
            S.beta(:,nvec)    = b;
...
%
    function UpdateIndep
        ...
        if order == 1
            ...
            indep = [ intercept  bat{2}(bc{1}) bat{3}(bc{2}) ];
        ...
        elseif order == 2
            ...
            indep = [intercept  bat{2}(bc{1}) bat{3}(bc{2}) bat{4}(bc{1},bc{2})];
            model = {bat{1} bat{2} bat{3} bat{4}};
        elseif order == 3
            %this is the full second order model:
            %That is a + b*x + c*y + d*x*y + e*x.*x + f*y*y.
            indep = [intercept  bat{2}(bc{1}) bat{3}(bc{2}) bat{4}(bc{1},bc{2}) bat{5}(bc{1}) bat{6}(bc{2}) ];
            model = {bat{1} bat{2} bat{3} bat{4} bat{5} bat{6}};
        end
