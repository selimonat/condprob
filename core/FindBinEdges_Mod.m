...
%         FeatureMap(:,ni) =  FeatureMap(:,ni) + rand(tPix,1)*u(i)/1000;        
        ...
        %Find the smallest non-zero entry's Index.We will use this index-1
        %to sum over all the fixation values to see the weight of the all
        %zero entryies.
        X = find(FeatM_s == min(FeatM_s(FeatM_s ~= 0)));               
        %
        %        
        ...
        %This is the total weight of the pixels which have an absolute value of 0
        T = sum(FixMap_sorted(1:X-1));
        %We have to know compute how much a given bin can take. This
        %depends on solely the number of bin.
        Capacity = 1/nBins;            
        %
        ...
        if T <= Capacity 
            ...
            BinEdges2(2) = FixMap_cs(X);
            Increment    = (FixMap_cs(end)-FixMap_cs(X))./(nBins-1);            
            BinEdges2    = [ BinEdges2 BinEdges2(2)+Increment*(1:(nBins-1))];
        end
        ...
        for x = 1:nBins  
            ...
            %
            x = nBins+1;
            BinEdges(x,ni) = FeatM_s(end);
            %why not replace this with BinEdges = FeatM_s(end)
    end
    
     if 1 == 0
         [n,F(:,i)] = histc(FeatM,BinEdges);
         c  = accumarray( F , FixMap(:) ,  [1 20]);
     end
